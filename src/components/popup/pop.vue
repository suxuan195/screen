<template>
  <div class="popup" v-show="props.showModel" @click.self="closeModel">
    <div class="box" :style="`width: ${width}px; height: ${height}px`">
      ...
      <div class="header1">
        <div class="title ttf">{{ props.title }}</div>
        <div style="margin-left: 20px;" v-if="isButtonShow">
          <span style="color:#fff">(2023年1{{ props.month === 1 ? '' : '-' + month }}月)</span>
          <el-button color="rgba(18, 50, 86)" @click="buttonClick(-1)" style="margin-left: 10px;">上个月</el-button>
          <el-button color="rgba(18, 50, 86)" @click="buttonClick(+1)">下个月</el-button>
        </div>
...
const closeModel = () => {
  emitBus.emit("closeModel");
};
const buttonClick = (val) => {
  emitBus.emit("buttonClick", val);
}
