<template>
  <div>
    <popup :width="954" :height="650" title="总装机量" class="popup1" :showModel="props.showModel">
...
const getList = async () => {
  ...
  state.list = [
    arr.find(item => item.type == '风电' && item.name == '总装机容量'),
    arr.find(item => item.type == '光伏' && item.name == '总装机容量'),
    arr.find(item => item.type == '水电' && item.name == '总装机容量'),
    arr.find(item => item.type == '火电' && item.name == '总装机容量'),
  ]
