<template>
  <div>
    <popup :width="954" :height="650" title="当年累计发电量" class="popup1" :showModel="props.showModel">
...
const getList = async () => {
  ...
  state.list = [
    arr.find(item => item.type == '风电' && item.name == '当年累计发电量'),
    arr.find(item => item.type == '光伏' && item.name == '当年累计发电量'),
    arr.find(item => item.type == '水电' && item.name == '当年累计发电量'),
    arr.find(item => item.type == '火电' && item.name == '当年累计发电量'),
  ]
  ...
  state.option = {
    tooltip: {
      ...
      formatter: function (params) {
        ...
        const color = state.colorEnum[params.dataIndex % 4];
  ...
  state.option1 = {
    ...
    tooltip: {
      ...
      formatter: function (p) {
        ...
        for (let i in p) {
          let item = p[i];
          str += `<div><span style="width:10px;height:10px;border-radius:50%;background-color:${state.colorList1[item.dataIndex%4]
