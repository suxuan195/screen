<template>
  <popup :width="1127" :height="456" :showModel="props.showModel" title="产权交易统计" class="popup1">
    <div class="tableX">
  ...
  borderBottom: '1px solid #1D80CF',
  borderLeft: '2px solid',
  borderRight: '1px solid',
  borderImage:
    'linear-gradient(180deg,  rgba(0, 155, 255, 0.1) 40%, rgba(0, 155, 255, 0.8) 100%) 2 2 2 2' /* 先设置透明的左边框 */,
}">
...
import { January, February, June, July } from "../temp/propertyTSData.js";
...
const getTableData = () => {
  ...
  if (props.modelId === "一月") {
    state.tableData = January;
  } else if (props.modelId === "二月") {
    state.tableData = February;
  } else if (props.modelId === "六月") {
    state.tableData = June;
  } else if (props.modelId === "七月") {
    state.tableData = July;
  } else {
    state.tableData = [];
  }
};
