<template>
  <div class="rightTopChart">
    ...
    <leftCenterDialog :tableHeader="state.tableHeader" ref="leftCenterDialogRef" />
  </div>
</template>
...
import leftCenterDialog from "../dialog/leftCenterDialog.vue";
  ...
  state.option = {
    tooltip: {
      ...
      formatter: function (params) {
        ...
        var tooltipContent = `${params.name}   ${params.data.percentage}`; // 自定义tooltip内容
    ...
    series: [
      {
        ...
        label: {
          ...
          formatter: (params) => {
            return `{b|${params.name}}  {a${params.dataIndex}|${params.value}人  ${params.percent}%}`;
...
const leftCenterDialogRef = ref(null);
const clickCallBack = (p) => {
  if (!p.name) return
  leftCenterDialogRef.value.showModel = true
  state.tableHeader = [
    {
      label: "企业名称",
      prop: "enterpriseName",
