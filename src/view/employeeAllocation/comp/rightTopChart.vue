<template>
  <div class="rightTopChart">
    ...
    <!-- <RightTopDialog ref="refRightTopDialog" :showModel="state.showModel"></RightTopDialog> -->
    <rightTopDialog ref="RightTopDialogRef" :tableHeader="state.tableHeader" />
  </div>
</template>

<script setup>
// import RightTopDialog from "./rightTopDialog.vue";
...
const clickCallBack = (p) => {
  ...
  state.tableHeader = [
    ...
    {
      label: '企业总人数',
      prop: 'enetrpriseTotal'
    },
    {
      label: `${p.seriesName}人数`,
      prop: 'otherTotal'
    },
    {
      label: '人数占比',
  ...
  for (let i = 0; i < state.list.length; i++) {
    ...
    seriesArr.push({
      ...
      data: [
        ...
        {
          name: "invisible",
          value: 100 - state.list[i].value - 1,
