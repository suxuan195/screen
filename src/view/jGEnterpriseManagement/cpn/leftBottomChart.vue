<template>
    <div class="rightBottomChart">
        <div>
            <p :class="index === state.counter ? 'active' : ''" v-for="(item, index) in  state.tabList" :key="index + '11'"
                @click="pClick(index)">{{ item }}</p>
        </div>
...
const init = () => {
    state.option = {
        ...
        legend: {
            data: [
                {
                    name: state.tabList[state.counter],
                    itemStyle: {
                        color: '#3C97F1',
                    },
                }
            ...
            formatter: function (p) {
                ...
                for (let i in p) {
                    ...
                    str += `<div><span style="width:10px;height:10px;border-radius:50%;
                        background-color:#3C97F1;margin-right:6px;display:inline-block;"></span>${state.tabList[state.counter]}：${item.data}</div>`;
                }
        ...
        yAxis: [
            {
                name: '单位(亿元)',
                // min: 0,
                // max: state.counter === 0 ? 250 : 10,
                // interval: state.counter === 0 ? 50 : 2.5,
                type: "value",
                splitLine: {
                    show: true,
                    lineStyle: {
        ...
        series: [
            {
                ...
                data: props.dataList[state.counter]
            },
        ],
        dataZoom: {
...
.rightBottomChart {
    ...
    div {
        ...
        p.active {
            ...
            border-image: linear-gradient(359deg, #166CFF -1%, rgba(209, 226, 255, 0.8701) 10%, rgba(129, 206, 255, 0.76) 97%) 1;
        }
    }
}
</style>