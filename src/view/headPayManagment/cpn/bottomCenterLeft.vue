    ...
    state.option = {
        ...
        tooltip: {
            ...
            formatter: function (p) {
                ...
                for (let i in p) {
                    let item = p[i];
                    str += `<div><span style="width:10px;height:10px;border-radius:50%;background-color:${state.colorList[item.seriesIndex % 3]
                        };margin-right:6px;display:inline-block;"></span>${item.data
                        } (${item.seriesIndex === 2 ? '万人' : '万元'})</div>`;
                }
