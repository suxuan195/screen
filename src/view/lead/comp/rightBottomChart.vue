<template>
  <div class="rightTopChart">
    ...
    <div class="chart">
      <my-chart :width="420" :height="382" :option="state.option" :length="state.list.length" :clickA="true"
        @clickACallBack="clickCallBack" :legendSelected="true" @legendSelectCallBack="clickCallBack"></my-chart>
    </div>
  </div>
  <teleport to="#screen">
    <RightBottomDialog ref="refRightBottomDialog"></RightBottomDialog>
  </teleport>
</template>
...
const refRightBottomDialog = ref(null);
...
const clickCallBack = async (p) => {
  refRightBottomDialog.value.showModel = true;
  await nextTick();
  refRightBottomDialog.value.setList(p.name);
};
  ...
  state.option = {
    ...
    tooltip: {
      ...
      formatter: function (params) {
        var tooltipContent = `${params.name} ${params.value
