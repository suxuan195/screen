<template>
  ...
  <teleport to="#screen">
    <CenterGeo2dDialog ref="refCenterGeo2dDialog"></CenterGeo2dDialog>
  </teleport>
</template>
...
import CenterGeo2dDialog from "../dialog/centerGeo2dDialog.vue";
import { useList } from "../hooks/map";
const { mapCount, getList } = useList();
const container = ref(null);
const refCenterGeo2dDialog = ref(null);
...
const init = () => {
  ...
  myChart.on("click", async (p) => {
    if (p.componentSubType == "effectScatter") {
      refCenterGeo2dDialog.value.showModel = true;
      await nextTick();
      refCenterGeo2dDialog.value.setList(p.name);
    }
  });
};
onMounted(async () => {
