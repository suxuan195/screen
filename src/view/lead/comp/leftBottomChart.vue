<template>
  <div class="leftTopChart">
    ...
    <div class="chart">
      <!-- <div class="bar">
        <div
          :class="`${index == state.tag ? 'active' : ''} item`"
          @click="selcetBar(index)"
          v-for="(item, index) in state.bar"
          :key="index"
        >
          {{ item }}
        </div> -->
      <!-- </div> -->
      <my-chart :width="420" :height="382" :option="state.option" :click="true" :params="_params"
        @clickCallBack="clickACallBack"></my-chart>
    </div>
    <teleport to="#screen">
      <leftBottomDialog ref="refLeftTopDialog" :showModel="state.showModel" :tableData="state.tableData">
      </leftBottomDialog>
    </teleport>
  </div>
</template>
...
import leftBottomDialog from "./leftBottomDialog.vue";
...
const init = () => {
  ...
  //   item.data = [];
  //   for (let i = 0; i < 7; i++) {
  //     let randomNum = Math.floor(Math.random() * 2301);
