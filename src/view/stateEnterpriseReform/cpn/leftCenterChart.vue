...
const state = reactive({
    ...
    title: ["已全面落实董事会职权的重要子企业户数比例", "已建立董事会向经理层授权管理制度的户数比例"],
    colorEnum: [
        '#3C97F1', '#0BE6E6'
    ],
    optionList: [
...
const init = () => {
    ...
    state.option.series.forEach((item) => {
        item.data = [];
        for (let i = 0; i < 7; i++) {
            let randomNum = Math.random() * 120.2;
