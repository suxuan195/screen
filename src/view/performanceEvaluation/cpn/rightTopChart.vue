<template>
    <div class="leftTopChart">
        ...
        <rightTopDialog ref="rightTopDialogRef" />
    </div>
</template>
...
const rightTopDialogRef = ref(null);
const title = ref('资产质量综合分析');
const state = reactive({
    ...
    title: ['总资产周转率', '应收账款周转率', '不良资产比率', '流动资产周转率'],
    colorList: ["#F6BB3B", "#3C97F1", "#6BDAFF", "#FC9202"],
    option1: {},
    title1: ['本月', '同比增长'],
    ...
    state.option = {
        ...
        tooltip: {
            ...
            formatter: function (p) {
                ...
                for (let i in p) {
                    let item = p[i];
                    str += `<div><span style="width:10px;height:10px;border-radius:50%;background-color:${state.colorList[item.seriesIndex % 4]
...
const tClick = () => {
    rightTopDialogRef.value.showModel = true
}
