...
const state = reactive({
    ...
    title: ['净资产收益率', '总资产报酬率', '营业利润率', '营业现金保障倍数', '成本费用利用率', '资本收益率'],
    colorList: ["#3C97F1", "#6BDAFF", "#F6BB3B", "#FC9202", "#5C6C80", "#23C99A"],
    option1: {},
    title1: ['本月', '同比增长'],
...
const legendData = state.title.map((item, index) => {
    ...
    return {
        name: item,
        itemStyle: { color: state.colorList[index] },
    }
})
const init = () => {
    state.option = {
